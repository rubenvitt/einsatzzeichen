 ...
 * `/docs/maplibre-lab/` auf 742 KB. Die Farbprofile bleiben eine Prop: die Seite wählt sie aus
 * `RENDER_THEMES` von `@einsatzzeichen/core` aus und benennt sie — es sind drei kleine Objekte,
 * die im HTML nichts kosten. (Bis LFH-570 lagen die Themes im Prüfpaket, dessen Index `node:url`
 * ins Browserbündel gezogen hätte; `core` ist browsertauglich.) Die Kartenbibliothek wird erst im Effekt geladen (`await import`), damit sie nicht im
