 ...
 *
 * Warum nicht `import.meta.glob`: 22 der Bögen tragen ein `#` im Dateinamen
 * (`recipe.D.1.9#alternative.svg`, die Alternativdarstellungen). Vite baut aus einem Glob-Treffer
 * einen Import-Spezifizierer, und dort beginnt hinter dem `#` ein Fragment — die 22 Dateien sind
 * über einen Glob nicht auflösbar (`UNRESOLVED_IMPORT`, belegt beim Bauen). Das Verzeichnis wird
 * deshalb mit `node:fs` gelesen und über eine statische Route ausgeliefert, die den Namen für die
 * URL entschärft. Nur so ist die Galerie vollständig.
 *
