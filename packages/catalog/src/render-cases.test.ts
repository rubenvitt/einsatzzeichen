...
describe('vollständige Renderfallmenge', () => {
  ...
  it('ist nicht leer und über die Implementierungs-ID eindeutig', () => {
    ...
    // 412 seit C.1.3 und I-a: 308 nach LFH-424, plus die 31 Zeichen aus E.2, alle 66
    // F-Rezepte, H.1 bis H.3, C.1.3 und die drei gemessenen Wasserfahrzeuge aus Anhang I.
    expect(ids).toHaveLength(412);
    ...
    // dazu 21 aus E-d, fünf aus E-e und fünf aus E-f. Anhang F ergänzt 66 weitere, H und I-a
    // jeweils drei; C.1.3 ergänzt einen weiteren Rezeptfall.
    expect(ids.filter((id) => id.startsWith('recipe.'))).toHaveLength(144);
