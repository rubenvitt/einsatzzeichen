...
describe('Fachreview-Ledger', () => {
  ...
  it('erfindet keine Fachfreigabe', () => {
    ...
    // Anhang H und I-a ergänzen je drei technisch belegte, fachlich weiterhin offene Zeichen;
    // C.1.3 ergänzt einen weiteren offenen Reviewplatz, Anhang N neun weitere.
    expect(manifestReviews).toHaveLength(440);
    expect(sourceReviews).toHaveLength(13);
    expect(profileReviews).toHaveLength(1);
    expect(reviews).toHaveLength(454);
