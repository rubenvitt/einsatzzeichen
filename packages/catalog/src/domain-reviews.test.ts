...
describe('Fachreview-Ledger', () => {
  ...
  it('erfindet keine Fachfreigabe', () => {
    ...
    // Anhang G ergänzt 21, H und I-a je drei, I-b vier und I-j drei, C.1.3 einen und Anhang N
    // neun offene Plätze; Anhang D ergänzt auf dem integrierten Stand weitere 36 Darstellungen.
    expect(manifestReviews).toHaveLength(504);
    expect(sourceReviews).toHaveLength(13);
    expect(profileReviews).toHaveLength(1);
    expect(reviews).toHaveLength(518);
