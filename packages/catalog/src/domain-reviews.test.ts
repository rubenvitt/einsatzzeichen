...
describe('Fachreview-Ledger', () => {
  ...
  it('führt für I-a und I-e genau acht weiterhin offene Fachreviews', () => {
    const keys = Object.keys(MANIFEST_DOMAIN_REVIEWS).filter((key) =>
      key.startsWith('bbk-babz-2025:I.1.') || key.startsWith('bbk-babz-2025:I.3.'),
    );
    expect(keys).toEqual([
  ...
  it('erfindet keine Fachfreigabe', () => {
    ...
    // Anhang G ergänzt 21, H, I-a und I-j je drei, I-e fünf, C.1.3 einen und Anhang N neun
    // offene Plätze;
    // Anhang D ergänzt auf dem integrierten Stand weitere 36 Darstellungen.
    expect(manifestReviews).toHaveLength(505);
    expect(sourceReviews).toHaveLength(13);
    expect(profileReviews).toHaveLength(1);
    expect(reviews).toHaveLength(519);
