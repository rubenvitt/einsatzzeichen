...
describe('coverage CLI', () => {
  it('weist alle Reviewträger getrennt aus und meldet null technische Nachweislücken', () => {
    ...
    // I-e ergänzt fünf Darstellungen auf 502.
    expect(manifestReviews).toBe(502);
    expect(sourceReviews).toBe(13);
    expect(profileReviews).toBe(1);
    expect(openReviews).toBe(516);
    ...
    expect(lines).toContain(
      'Umfang:      1, 2, 4, 5.1.1, 5.4, 5.8, C.1.1, C.1.2, C.1.3, D, E, F, G, H, I.1.9, I.1.10, I.1.11, I.1.12, I.3.5, I.3.6, I.3.7, J.1, J.2, J.3, J.4, K, L, M, N',
    );
