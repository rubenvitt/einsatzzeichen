...
describe('coverage CLI', () => {
  it('weist alle Reviewträger getrennt aus und meldet null technische Nachweislücken', () => {
    ...
    // LFH-489 ergänzt exakt I.5.1 bis I.5.3 und damit drei weitere Manifestreviews: 500.
    expect(manifestReviews).toBe(500);
    expect(sourceReviews).toBe(13);
    expect(profileReviews).toBe(1);
    expect(openReviews).toBe(514);
    ...
    expect(lines).toContain(
      'Umfang:      1, 2, 4, 5.1.1, 5.4, 5.8, C.1.1, C.1.2, C.1.3, D, E, F, G, H, I.3.5, I.3.6, I.3.7, I.5.1, I.5.2, I.5.3, J.1, J.2, J.3, J.4, K, L, M, N',
    );
