...
describe('coverage CLI', () => {
  it('weist alle Reviewträger getrennt aus und meldet null technische Nachweislücken', () => {
    ...
    // acht Zeichen F.3.12 bis F.3.19 ab: insgesamt 424. Anhang H ergänzt drei
    // veterinärmedizinische Formationen auf insgesamt 427.
    expect(manifestReviews).toBe(427);
    expect(sourceReviews).toBe(13);
    expect(profileReviews).toBe(1);
    expect(openReviews).toBe(441);
    ...
    expect(lines).toContain(
      'Umfang:      1, 2, 4, 5.1.1, 5.4, 5.8, C.1.1, C.1.2, D.3.7, E, F, H, J.1, J.2, J.3, J.4, K, L, M',
    );
