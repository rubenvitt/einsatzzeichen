...
describe('coverage CLI', () => {
  it('weist alle Reviewträger getrennt aus und meldet null technische Nachweislücken', () => {
    ...
    // I-b ergänzt I.2.4 bis I.2.7 als vier Anhänger auf 501 Zeilen; I-j ergänzt I.4.1 bis I.4.3
    // als drei literale Zeilen auf insgesamt 504.
    expect(manifestReviews).toBe(504);
    expect(sourceReviews).toBe(13);
    expect(profileReviews).toBe(1);
    expect(openReviews).toBe(518);
    ...
    expect(lines).toContain(
      'Umfang:      1, 2, 4, 5.1.1, 5.4, 5.8, C.1.1, C.1.2, C.1.3, D, E, F, G, H, I.2.4, I.2.5, I.2.6, I.2.7, I.3.5, I.3.6, I.3.7, I.4.1, I.4.2, I.4.3, J.1, J.2, J.3, J.4, K, L, M, N',
    );
