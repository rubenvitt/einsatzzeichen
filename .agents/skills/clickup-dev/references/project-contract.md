...
8. Stop: do not merge when any required local gate or exact-HEAD CI is RED, failed, missing, or stale. Approval never overrides this stop condition.
9. Merge only an approved tuple with current green local gates and exact-HEAD CI; run fresh gates against effective remote `main`.
10. Mark exactly that subtask `shipped` only after the remote-main evidence is green.

This workflow never automatically transitions any task to `done`; `done` is a separate later manual acceptance decision. Read live status names rather than deriving replacements from prose. Keep authentication material out of tasks, templates, PRs, screenshots, and committed files. Start a subtask with [the subtask template](../templates/clickup-subtask.md).
